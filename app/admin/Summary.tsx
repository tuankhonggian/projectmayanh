...
import { fortmatNumber } from "@/utils/formatNumber";
...
const Summary: React.FC<SummaryProps> = ({ orders, products, users }) => {
    const [summaryData, setSummaryData] = useState<SummaryDataType>({
        ...
        paidOrders: {
            label: 'Đơn đặt hàng đã thanh toán',
            digit: 0
        },
        unpaidOrders: {
                    ...
                    summaryKeys && summaryKeys.map((key) => {
                        ...
                        ">
                            <div className="text-xl md:text-4xl font-bold">
                                {
                                    summaryData[key].label === 'Tổng doanh thu' ?
                                        <>{formatPrice(summaryData[key].digit)}
                                        </> : <>
                                            {fortmatNumber(summaryData[key].digit)}
                                        </>
                                }
                            </div>
                            <div>
